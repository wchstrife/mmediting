...
class InductiveFilterCascade(BaseMattor):
    ...
    def __init__(self,
        ...
        super(InductiveFilterCascade, self).__init__(backbone, None, train_cfg, test_cfg,
                                       pretrained)
        self.backbone_2 = (
            build_backbone(backbone_2) if backbone_2 is not None else None)
        if self.backbone_2 is not None:
            self.backbone_2.init_weights(pretrained)
        self.loss_alpha = (
    ...
    def forward_dummy(self, inputs):
        trimap_p = self.backbone(inputs)
        if trimap_p.shape[-2:] != inputs.shape[-2:]:
            trimap_p = F.interpolate(trimap_p, scale_factor=4, mode='bilinear', align_corners=False)
        trimap_p = F.softmax(trimap_p, dim=1)
        _, unsure, fs = torch.split(trimap_p, 1, dim=1)
        alpha_r = self.backbone_2(torch.cat((inputs[:, :3, :, :], trimap_p), 1))
    ...
    def train_step(self, data_batch, optimizer):
        ...
        if self.backbone_2 is not None:
            optimizer['backbone_2'].zero_grad()
        loss.backward()
        optimizer['backbone'].step()
        if self.backbone_2 is not None:
            optimizer['backbone_2'].step()
    ...
    def forward_train(self, merged, mask, meta, alpha, trimap, fg=None, bg=None, ori_merged=None):
        ...
        trimap_p = self.backbone(torch.cat((merged, mask), 1))
        if trimap_p.shape[-2:] != merged.shape[-2:]:
            trimap_p = F.interpolate(trimap_p, merged.shape[-2:], mode='bilinear', align_corners=False)
        trimap = trimap // 125
        ...
        if self.backbone_2 is not None:
            ...
            trimap_p = F.softmax(trimap_p, dim=1)
            _, unsure, fs = torch.split(trimap_p, 1, dim=1)
            alpha_r = self.backbone_2(torch.cat((merged, trimap_p), 1))
            alpha_p = fs+unsure*alpha_r
        else:    
            losses['loss_trimap'] =  F.cross_entropy(trimap_p, trimap.squeeze(1).long(), reduction='none').mean()
    ...
    def forward_test(self,
        ...
        if trimap_p.shape[-2:] != merged.shape[-2:]:
            trimap_p = F.interpolate(trimap_p, merged.shape[-2:], mode='bilinear', align_corners=False)
        trimap_p = F.softmax(trimap_p, dim=1)
        _, unsure, fg = torch.split(trimap_p, 1, dim=1)
        alpha_r = self.backbone_2(torch.cat((merged, trimap_p), 1))
        if self.constraint_loss is not None:
            trimap_index = torch.argmax(trimap_p, dim=1, keepdim=False).cpu().numpy().squeeze()
            pred_alpha = alpha_r.cpu().numpy().squeeze()
            pred_alpha = np.clip(pred_alpha, 0, 1)
            pred_alpha[trimap_index==0]=0
            pred_alpha[trimap_index==2]=1
        else:
            alpha_p = fg+unsure*alpha_r
            pred_alpha = alpha_p.cpu().numpy().squeeze()
            pred_alpha = np.clip(pred_alpha, 0, 1)
        meta[0]['ori_trimap'] = np.ones_like(meta[0]['ori_mask']) * 128
