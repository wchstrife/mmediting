# fba decoder separate：将最后conv的输出变为两只，一支负责预测fg bg，一支预测alpha
import torch
...
class FBADecoderSeparate(nn.Module):

    def __init__(self, batch_norm=False):   # 默认使用GN
        super(FBADecoderSeparate, self).__init__()
        pool_scales = (1, 2, 3, 6)
        ...
        self.conv_up5 = nn.Sequential(
            ...
            nn.Conv2d(32, 16, kernel_size=3, padding=1, bias=True),
            nn.LeakyReLU(),
            nn.Conv2d(16, 6, kernel_size=1, padding=0, bias=True)
        )
    ...
    def forward(self, conv_out, img, indices, two_chan_trimap):
        ...
        alpha = self.conv_up4(x)
        FB = self.conv_up5(x)

        # import numpy as np
        # temp1 = output[:, 0][:, None].cpu().numpy().squeeze()
        # temp2 = output[:, 1:4].cpu().numpy().squeeze()
        ...
        # alpha = output[:, 0][:, None]
        F = torch.sigmoid(FB[:, 0:3])
        B = torch.sigmoid(FB[:, 3:6])
        output = torch.cat((alpha, F, B), 1)
