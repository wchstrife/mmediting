...
lr_config = dict(policy='Step', step=[(64000 * 40) // (4*4)], gamma=0.1, by_epoch=False)    # 共63960

# checkpoint saving
checkpoint_config = dict(interval=(64000) // (4*4), by_epoch=False)
evaluation = dict(interval=(64000 * 80) // (4*4), save_image=False)
# yapf:disable
log_config = dict(
    interval=50,
    hooks=[
...
# runtime settings
total_iters = (64000 * 80) // (4*4)
dist_params = dict(backend='nccl')
log_level = 'INFO'
work_dir = './work_dirs/fba/train'
load_from = None #'work_dirs/fba/mx-gn1-gpu6/iter_828000.pth'     # best model
resume_from = None
workflow = [('train', 1)]
