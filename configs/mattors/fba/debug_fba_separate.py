...
model = dict(
    ...
    backbone=dict(
        ...
        decoder=dict(type='FBADecoderSeparate')),
...
lr_config = dict(policy='Step', step=[(43100*40)//(4*4)], gamma=0.1, by_epoch=False)

# checkpoint saving
checkpoint_config = dict(interval=(43100)//(4*4), by_epoch=False)
evaluation = dict(interval=(43100*80)//(4*4), save_image=False)
# yapf:disable
log_config = dict(
    interval=50,
    hooks=[
