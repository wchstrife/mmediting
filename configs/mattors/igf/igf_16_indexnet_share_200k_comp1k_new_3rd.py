...
model = dict(
    type='MaskMatting',
    backbone=dict(
        type='SimpleShareEncoderDecoder',
        encoder=dict(type='IndexNetShareEncoder', in_channels=3, freeze_bn=True,
            index_mode='holistic', use_context=False, aspp=False,
            norm_cfg=dict(type='SyncBN', requires_grad=True)),
        neck=dict(type='IndexNetNeck', in_channels=4, freeze_bn=True,
            index_mode='holistic',
            norm_cfg=dict(type='SyncBN', requires_grad=True), 
            pretrained=None),
        decoder1=dict(
        ...
        decoder2=dict(type='IndexNetDecoder',
                norm_cfg=dict(type='SyncBN', requires_grad=True)),
        train_encoder=True,
        train_decoder1=True),
...
data_root = 's3://3darseg.matting/matting_data/matting_data/'

img_norm_cfg = dict(
    mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225], to_rgb=True)
train_pipeline = [
    dict(type='LoadImageFromFile', key='alpha', flag='grayscale', io_backend='petrel'),
    dict(type='LoadImageFromFile', key='fg', io_backend='petrel'),
    dict(type='LoadImageFromFile', key='bg', io_backend='petrel'),
    dict(type='LoadImageFromFile', key='merged', save_original_img=True, io_backend='petrel'),
    dict(type='LoadImageFromFile', key='trimap', flag='grayscale', io_backend='petrel'),
    dict(
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file='data/adobeclearhair_train.json',
        data_prefix=data_root,
        pipeline=train_pipeline),
    val=dict(
        type=dataset_type,
        ann_file='data/hair_test.json',
        data_prefix=data_root,
...
optimizers = dict(
    ...
    type='Adam',
    lr=1e-2,
    paramwise_cfg=dict(custom_keys={'encoder.layers': dict(lr_mult=0.01), 'neck.layers': dict(lr_mult=0.01)}))
...
load_from = './work_dirs/igf_cascade_pretrain6/share_2nd_2/iter_36000.pth'
